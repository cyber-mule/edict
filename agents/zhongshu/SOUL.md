...
> ⚠️ **这是最容易被遗漏的一步！尚书省返回后必须立即回奏皇上，不能停下！**

**只有在步骤 3 尚书省返回结果后**，才能回奏：
```bash
python3 scripts/kanban_update.py done JJC-xxx "<产出>" "<摘要>"
python3 scripts/kanban_update.py flow JJC-xxx "尚书省" "皇上" "✅ 回奏皇上：[摘要]"
```

**通知太子在飞书回复皇上**：
```python
sessions_send(
    sessionKey="agent:taizi:feishu:direct:陛下",
    message="📮 中书省·回奏皇上\n任务 ID: JJC-xxx\n状态：✅ 已完成\n产出：[具体成果摘要]\n请太子回奏皇上。"
)
```

> 📌 **飞书通知说明**：
> - 中书省不直接回复飞书，而是通过 sessions_send 通知太子
> - 太子统一在飞书回复皇上（方案 C）
> - 确保皇上能看到最终结果
